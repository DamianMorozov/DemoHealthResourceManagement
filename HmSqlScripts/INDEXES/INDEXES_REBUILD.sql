...
END ELSE BEGIN
	...
	END ELSE BEGIN
		...
		---------------------------------------------------------------------------------------------------------------------
		-- [EMPLOYEES] & [DT_CREATE]
		IF ((SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_EMPLOYEES_DT_CREATE') IS NOT NULL) BEGIN
			ALTER INDEX [AK_EMPLOYEES_DT_CREATE] ON [REF].[EMPLOYEES] REBUILD PARTITION = ALL 
				WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON);
			PRINT N'[✓] REBUILD INDEX [AK_PERSONS_DT_CREATE]';
		END;
		-- [EMPLOYEES] & [DT_CHANGE]
		IF ((SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_EMPLOYEES_DT_CHANGE') IS NOT NULL) BEGIN
			ALTER INDEX [AK_EMPLOYEES_DT_CHANGE] ON [REF].[EMPLOYEES] REBUILD PARTITION = ALL 
				WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON);
			PRINT N'[✓] REBUILD INDEX [AK_PERSONS_DT_CHANGE]';
		END;
		-- [EMPLOYEES] & [PERSON_ID]
