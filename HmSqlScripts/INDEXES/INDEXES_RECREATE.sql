...
END ELSE BEGIN
	...
	END ELSE BEGIN
		...
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [PERSONS] & [CELLPHONE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_PERSONS_CELLPHONE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[PERSONS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'AK_PERSONS_CELLPHONE';
		SET @CMD = 'CREATE UNIQUE INDEX [' + @INDEX + '] ON [REF].[PERSONS] ([CELLPHONE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [PERSONS] & [EMAIL]
		...
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [PERSONS] & [INN]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_PERSONS_INN');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[PERSONS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'AK_PERSONS_INN';
		SET @CMD = 'CREATE UNIQUE INDEX [' + @INDEX + '] ON [REF].[PERSONS] ([INN])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		---------------------------------------------------------------------------------------------------------------------
		-- DOCTORS
		PRINT N'[✓] TABLE [DOCTORS]';
		-- [DOCTORS] & [DT_CREATE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'IX_DOCTORS_DT_CREATE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[DOCTORS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_DOCTORS_DT_CREATE';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [REF].[DOCTORS] ([DT_CREATE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [DOCTORS] & [DT_CHANGE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'IX_DOCTORS_DT_CHANGE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[DOCTORS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_DOCTORS_DT_CHANGE';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [REF].[DOCTORS] ([DT_CHANGE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [DOCTORS] & [PERSON_ID]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_DOCTORS_PERSON_ID');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[DOCTORS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'AK_DOCTORS_PERSON_ID';
		SET @CMD = 'CREATE UNIQUE INDEX [' + @INDEX + '] ON [REF].[DOCTORS] ([PERSON_ID])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		---------------------------------------------------------------------------------------------------------------------
		-- MEDICAL_CARDS
		...
		---------------------------------------------------------------------------------------------------------------------
		-- PATIENTS
		PRINT N'[✓] TABLE [PATIENTS]';
		-- [PATIENTS] & [DT_CREATE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'IX_PATIENTS_DT_CREATE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[PATIENTS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_PATIENTS_DT_CREATE';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [REF].[PATIENTS] ([DT_CREATE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [PATIENTS] & [DT_CHANGE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'IX_PATIENTS_DT_CHANGE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[PATIENTS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_PATIENTS_DT_CHANGE';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [REF].[PATIENTS] ([DT_CHANGE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [PATIENTS] & [PERSON_ID]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_PATIENTS_PERSON_ID');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[PATIENTS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'AK_PATIENTS_PERSON_ID';
		SET @CMD = 'CREATE UNIQUE INDEX [' + @INDEX + '] ON [REF].[PATIENTS] ([PERSON_ID])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [PATIENTS] & [MEDICAL_CARD_ID]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_REF_NAME AND [INDEX] = 'AK_PATIENTS_MEDICAL_CARD_ID');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [REF].[PATIENTS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'AK_PATIENTS_MEDICAL_CARD_ID';
		SET @CMD = 'CREATE UNIQUE INDEX [' + @INDEX + '] ON [REF].[PATIENTS] ([MEDICAL_CARD_ID])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		---------------------------------------------------------------------------------------------------------------------
		-- RECEPTIONS
		PRINT N'[✓] TABLE [RECEPTIONS]';
		-- [RECEPTIONS] & [DT_CREATE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_DIM_NAME AND [INDEX] = 'IX_RECEPTIONS_DT_CREATE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [DIM].[RECEPTIONS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_RECEPTIONS_DT_CREATE';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [DIM].[RECEPTIONS] ([DT_CREATE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [RECEPTIONS] & [DT_CHANGE]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_DIM_NAME AND [INDEX] = 'IX_RECEPTIONS_DT_CHANGE');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [DIM].[RECEPTIONS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_RECEPTIONS_DT_CHANGE';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [DIM].[RECEPTIONS] ([DT_CHANGE])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [RECEPTIONS] & [DT_START]
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_DIM_NAME AND [INDEX] = 'IX_RECEPTIONS_DT_START');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [DIM].[RECEPTIONS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'IX_RECEPTIONS_DT_START';
		SET @CMD = 'CREATE INDEX [' + @INDEX + '] ON [DIM].[RECEPTIONS] ([DT_START])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		---------------------------------------------------------------------------------------------------------------------
	END;
