...
END ELSE BEGIN
	...
	END ELSE BEGIN
		...
		SET @INDEX = (SELECT [INDEX] FROM @INDEXES WHERE [SCHEMA] = @SCHEMA_DIM_NAME AND [INDEX] = 'AK_RECEPTIONS_PATIENT_ID');
		IF (@INDEX IS NOT NULL) BEGIN
			SET @CMD = 'DROP INDEX IF EXISTS [' + @INDEX + '] ON [DIM].[RECEPTIONS]';
			EXEC (@CMD);
			PRINT N'[✓] INDEX DROPPED [' + @INDEX + N']';
		END;
		SET @INDEX = 'AK_RECEPTIONS_PATIENT_ID';
		SET @CMD = 'CREATE UNIQUE INDEX [' + @INDEX + '] ON [DIM].[RECEPTIONS] ([PATIENT_ID])';
		EXEC (@CMD);
		PRINT N'[✓] INDEX CREATED [' + @INDEX + N']';
		-- [RECEPTIONS] & [DT_START]
