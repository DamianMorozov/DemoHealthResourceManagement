	...
	END ELSE BEGIN
		...
		IF EXISTS (SELECT 1 FROM [SYS].[TABLES] WHERE [SCHEMA_ID] = @SCHEMA_ID AND [NAME] = N'MEDICAL_CARDS') BEGIN
			...
			IF NOT EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[TABLE_CONSTRAINTS]
				WHERE [CONSTRAINT_TYPE] = 'PRIMARY KEY' AND [TABLE_SCHEMA] = @SCHEMA_NAME AND [TABLE_NAME] = 'MEDICAL_CARDS') BEGIN
				ALTER TABLE [REF].[MEDICAL_CARDS] ADD CONSTRAINT [PK_MEDICAL_CARDS_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH 
					(PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [FG_REF];
				PRINT N'[✓] PRIMARY KEY [PK_MEDICAL_CARDS_ID] WAS CREATED';
			END;
			-- DEFAULT
			IF NOT EXISTS (SELECT 1 FROM [SYS].[DEFAULT_CONSTRAINTS] WHERE [schema_id] = @SCHEMA_ID AND [NAME] = 'DF_MEDICAL_CARDS_DT_CREATE') BEGIN
				ALTER TABLE [REF].[MEDICAL_CARDS] ADD CONSTRAINT [DF_MEDICAL_CARDS_DT_CREATE] DEFAULT (GETDATE()) FOR [DT_CREATE];
				PRINT N'[✓] CONSTRAINT [DF_MEDICAL_CARDS_DT_CREATE] WAS CREATED';
			END;
			IF NOT EXISTS (SELECT 1 FROM [SYS].[DEFAULT_CONSTRAINTS] WHERE [schema_id] = @SCHEMA_ID AND [NAME] = 'DF_MEDICAL_CARDS_DT_CHANGE') BEGIN
				ALTER TABLE [REF].[MEDICAL_CARDS] ADD CONSTRAINT [DF_MEDICAL_CARDS_DT_CHANGE] DEFAULT (GETDATE()) FOR [DT_CHANGE];
				PRINT N'[✓] CONSTRAINT [DF_MEDICAL_CARDS_DT_CHANGE] WAS CREATED';
			END;
		END;
	END;
	-- COMMIT
