...
END ELSE BEGIN
	...
	END ELSE BEGIN
		...
		-- CREATE TABLE
		IF NOT EXISTS (SELECT 1 FROM [SYS].[TABLES] WHERE [SCHEMA_ID] = @SCHEMA_ID AND [name] = N'MEDICAL_CARDS') BEGIN
			CREATE TABLE [REF].[MEDICAL_CARDS] (
				[ID] [INT] IDENTITY(1,1) NOT NULL,
				[DT_CREATE] [DATETIME] NOT NULL,
				[DT_CHANGE] [DATETIME] NOT NULL,
				[DESCRIPTION] [NVARCHAR](MAX) NOT NULL,
			) ON [FG_REF];
			PRINT N'[✓] CREATED TABLE [' + @SCHEMA_NAME + '].[MEDICAL_CARDS]';
		END;
	END;
	-- COMMIT
